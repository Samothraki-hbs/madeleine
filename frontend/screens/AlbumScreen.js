...
export default function AlbumScreen({ route }) {
  ...
  const fetchPhotos = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.0.20:3000/albums/${albumId}/photos`, {
  ...
  const fetchPhotosToSort = async () => {
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(`http://192.168.0.20:3000/albums/${albumId}/photos-to-sort`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
  ...
  const handleSort = async (status) => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      if (status === 'pinned') {
        await fetch(`http://192.168.0.20:3000/photos/${photo.photoId}/pin`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ albumId }),
        });
      } else {
        await fetch(`http://192.168.0.20:3000/photos/${photo.photoId}/status`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
